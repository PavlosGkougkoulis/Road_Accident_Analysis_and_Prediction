...
df = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Weekly_LSTM/Accidents_LSTM_Prediction/Accident_Forecast_DFs/Accidents_Forecast_Df.xlsx")
...
plt.title("Actual vs Predicted Accidents (2018+)")
...
plt.tight_layout()
plt.show()
rolling_error = pd.Series(np.abs(residuals)).rolling(window=4).mean()  # 4-week MA
plt.figure(figsize=(10, 4))
plt.plot(rolling_error, label="4-Week Rolling MAE", color="orange")
plt.title("Rolling Mean Absolute Error (4-week)")
plt.xlabel("Weeks (Test Set)")
