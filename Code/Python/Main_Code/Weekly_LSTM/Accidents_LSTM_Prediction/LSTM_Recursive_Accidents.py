...
df_base = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Weekly_LSTM/Accidents_LSTM_Prediction/Accident_Forecast_DFs/Accidents_Forecast_Df.xlsx")
df_frozen = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Weekly_LSTM/Accidents_LSTM_Prediction/Accident_Forecast_DFs/Accidents_Forecast_Df_Frozen.xlsx")
...
for i in range(last_known_index + 1, len(df_frozen)):
    ...
    for w in [4, 7, 12, 24, 52]:
        df_frozen[f'Rolling_Avg_{w}w_a'] = df_frozen['accidents'].rolling(window=w, min_periods=1).mean()
    for w in [4, 8, 12, 24]:
        df_frozen[f'RollingSum_{w}w_a'] = df_frozen['accidents'].rolling(window=w, min_periods=1).sum()
...
df_frozen.to_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Weekly_LSTM/Accidents_LSTM_Prediction/Recursive_Prediction_Accidents/Recursive_Forecast_Accidents.xlsx", index=False)
print("📁 Saved full recursive forecast to 'Recursive_Forecast_Accidents.xlsx'")


# Step 1: Load actual data and sort

df_actual = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Weekly_LSTM/Accidents_LSTM_Prediction/Accident_Forecast_DFs/Accidents_Forecast_Df.xlsx")
...
plt.plot(df_all["week_date"], df_all["accidents"], label="Accidents (Actual + Forecast)", color='blue')
plt.axvline(x=last_date, color='red', linestyle='--', label="Forecast Start (2023)")
plt.title("Weekly Accidents in Greece (1996–2026)", fontsize=14)
