...
acc_m_ind_y_mod = pd.read_csv("E:/University/Thesis/Thesis_Main/Thesis_Code/Dfs/Dfs_LSTM/Monthly_Accident_Dataset_LSTM.csv")


#Keep only selected columns

acc_m_ind_y_mod = acc_m_ind_y_mod[[
    'year', 'month',
...
acc_m_ind_y_mod['quarter'] = ((acc_m_ind_y_mod['month'] - 1) // 3) + 1
...
acc_m_ind_y_mod['season'] = acc_m_ind_y_mod['month'].apply(get_season_code)

#Set is_summer and is_winter flags as 1/0

acc_m_ind_y_mod['is_summer'] = (acc_m_ind_y_mod['season'] == 3).astype(int)
acc_m_ind_y_mod['is_winter'] = (acc_m_ind_y_mod['season'] == 1).astype(int)
def compute_month_date(row):
    day = {1: 1}.get(int(row['month']), 1)
    return pd.Timestamp(year=int(row['year']), month=int(row['month']), day=day)

#Apply to Df

acc_m_ind_y_mod['month_date'] = acc_m_ind_y_mod.apply(compute_month_date, axis=1)
...
def mark_greek_holidays(df, date_col='month_date'):
    df = df.copy()
    ...
    return df
acc_m_ind_y_mod = mark_greek_holidays(acc_m_ind_y_mod, date_col='month_date')

#Lags

lag_months = [1,2, 3, 4, 6, 7, 8, 12, 24]
for lag in lag_months:
    acc_m_ind_y_mod[f'Accidents_Lag{lag}'] = acc_m_ind_y_mod['accidents'].shift(lag)
    acc_m_ind_y_mod[f'Fatalities_Lag{lag}'] = acc_m_ind_y_mod['fatalities'].shift(lag)

#Rolling Averages

rolling_windows = [4, 7, 12]
for w in rolling_windows:
    acc_m_ind_y_mod[f'Rolling_Avg_{w}m_a'] = acc_m_ind_y_mod['accidents'].rolling(window=w, min_periods=1).mean()
for w in rolling_windows:
    acc_m_ind_y_mod[f'Rolling_Avg_{w}m_f'] = acc_m_ind_y_mod['fatalities'].rolling(window=w, min_periods=1).mean()

#Rolling Sums

rolling_sum_windows = [4, 8, 12, 24]
for w in rolling_sum_windows:
    acc_m_ind_y_mod[f'RollingSum_{w}m_a'] = acc_m_ind_y_mod['accidents'].rolling(window=w, min_periods=1).sum()
for w in rolling_sum_windows:
    acc_m_ind_y_mod[f'RollingSum_{w}m_f'] = acc_m_ind_y_mod['fatalities'].rolling(window=w, min_periods=1).sum()

#Cumulative Sums

acc_m_ind_y_mod['CumulativeAccidentsY'] = acc_m_ind_y_mod.groupby('year')['accidents'].cumsum()
acc_m_ind_y_mod['CumulativeFatalitiesY'] = acc_m_ind_y_mod.groupby('year')['fatalities'].cumsum()
acc_m_ind_y_mod['CumulativeAccidents'] = acc_m_ind_y_mod['accidents'].cumsum()
acc_m_ind_y_mod['CumulativeFatalities'] = acc_m_ind_y_mod['fatalities'].cumsum()
...
df['covid'] = np.where((df['month_date'] >= covid_start) & (df['month_date'] <= covid_end), 1, 0)
...
acc_m_ind_y_mod = df
acc_m_ind_y_mod.to_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Monthly_LSTM/Fatalities_LSTM_Prediction/Fatalities_Forecast_DFs/Fatalities_Forecast_Df.xlsx", index=False)
...
results_df.to_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Monthly_LSTM/Fatalities_LSTM_Prediction/LSTM_Hparams_Evaluation_Fatalities/LSTM_Gridsearch_Results_Fatalities.xlsx", index=False)
print("📁 Grid search results saved as 'LSTM_Gridsearch_Results_Fatalities.xlsx'")
