...
df_base = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Monthly_LSTM/Fatalities_LSTM_Prediction/Fatalities_Forecast_DFs/Fatalities_Forecast_Df.xlsx")
df_frozen = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Monthly_LSTM/Fatalities_LSTM_Prediction/Fatalities_Forecast_DFs/Fatalities_Forecast_Df_Frozen.xlsx")
...
for i in range(last_known_index + 1, len(df_frozen)):
    ...
    for w in [4, 7, 12]:
        df_frozen[f'Rolling_Avg_{w}m_f'] = df_frozen['fatalities'].rolling(window=w, min_periods=1).mean()
    for w in [3, 6, 12, 24]:
        df_frozen[f'RollingSum_{w}m_f'] = df_frozen['fatalities'].rolling(window=w, min_periods=1).sum()
...
df_frozen.to_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Monthly_LSTM/Fatalities_LSTM_Prediction/Recursive_Prediction_Fatalities/Recursive_Forecast_Fatalities.xlsx", index=False)
print("📁 Saved full recursive forecast to 'Recursive_Forecast_Fatalities.xlsx'")


# Step 1: Load actual data and sort

df_actual = pd.read_excel("E:/University/Thesis/Thesis_Main/Thesis_Code/Python/Main_Code/Monthly_LSTM/Fatalities_LSTM_Prediction/Fatalities_Forecast_DFs/Fatalities_Forecast_Df.xlsx")
...
plt.plot(df_all["month_date"], df_all["fatalities"], label="Fatalities (Actual + Forecast)", color='blue')
plt.axvline(x=last_date, color='red', linestyle='--', label="Forecast Start (2023)")
plt.title("Monthly Fatalities in Greece (1996–2026)", fontsize=14)
